...
const James = () => {
    ...
    useEffect(()=>{
        jamesAnims = {
            filterAnim: gsap.to(ref.current['jamesBody'], { filter: "grayscale(0)", paused: true }),
            armAnim: gsap.to(ref.current['arm'], { rotation: 82, x: -100, duration: 2, transformOrigin: "100% bottom", paused: true }),
            headAnim: gsap.to(ref.current['head'], { rotation: -15, x: -25, delay: 0.25, duration: 1.75, paused: true })
        }
    },[])
    ...
    return (
        <JamesContainer>
            <JamesBody ref={(element) => {
                ref.current["jamesBody"] = element;
            }}>
                <TargetUI top={185} right={-460} onMouseEnter={jamesHover} onMouseLeave={jamesHover}/>
                <Head ref={(element) => {
